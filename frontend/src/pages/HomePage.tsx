...
import {
  Shield, FileSearch, Layers, Zap, Brain, CheckCircle2,
  ArrowRight, Database, Bot, MessageSquare, Eye, ChevronRight,
  Lock, BarChart3, BookOpen, Users, Globe, Building2, Play,
  Quote, AlertTriangle, Search, Activity, Cpu, Leaf, Menu, X
} from "lucide-react";
...
function MarketingNav() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-xl border-b border-border/50">
      ...
      {mobileOpen && (
        <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="md:hidden border-t bg-background px-6 py-4 space-y-3">
          {["Product", "Solutions", "Resources", "Company", "Pricing"].map((item) => (
            <a key={item} href={`#${item.toLowerCase()}`} className="block text-sm text-foreground py-2" onClick={() => setMobileOpen(false)}>
              {item}
            </a>
          ))}
          <div className="flex gap-3 pt-2">
            <Link to="/login" className="flex-1"><Button variant="outline" className="w-full rounded-full">Log in</Button></Link>
            <Link to="/login" className="flex-1"><Button className="w-full rounded-full bg-foreground text-background">Try free</Button></Link>
          </div>
        </motion.div>
      )}
    </header>
      ...
      <div className="max-w-7xl mx-auto px-6 relative">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <motion.div initial="hidden" animate="visible" className="max-w-xl">
            ...
            <motion.h1 variants={fadeUp} custom={1} className="text-5xl md:text-6xl lg:text-[4.25rem] font-serif leading-[1.08] text-foreground mb-6">
            ...
            <motion.p variants={fadeUp} custom={2} className="text-lg text-muted-foreground leading-relaxed mb-8 max-w-md">
              Build agents that reason over your documents, policies, specifications, and institutional knowledge — with citations, confidence, and traceability.
            </motion.p>
          ...
          >
            <img src={heroVisual} alt="Abstract visual representing grounded intelligence" className="w-full rounded-2xl shadow-float" />
          </motion.div>
        </div>
      </div>
    </section>
  ...
  return (
    <section className="py-12 border-y border-border/50">
      <div className="max-w-7xl mx-auto px-6">
        <p className="text-center text-xs tracking-widest uppercase text-muted-foreground mb-6">Trusted by teams building on critical knowledge</p>
        <div className="flex flex-wrap justify-center items-center gap-x-12 gap-y-4 opacity-40">
          {["Enterprise Co.", "TechCorp", "Legal Partners LLP", "Global Compliance", "InnovateLab", "SecureOps"].map(name => (
            <span key={name} className="text-sm font-semibold text-foreground tracking-wide">{name}</span>
          ))}
        </div>
      </div>
    </section>
...
function CapabilitiesSection() {
  const features = [
    { icon: Database, title: "Dataset-Backed Intelligence", desc: "Organize documents into datasets as the source of truth. Index specifications, policies, logs, and any institutional knowledge." },
    { icon: Bot, title: "Grounded Agents", desc: "Build agents that reason over your datasets. Each agent can access multiple datasets and maintain conversation context." },
    { icon: Layers, title: "Evidence-First Retrieval", desc: "Every answer starts with evidence. The system retrieves relevant passages before generating, ensuring answers are grounded in your data." },
    { icon: Quote, title: "Sentence-Level Citations", desc: "Inspect exactly where answers come from. Citations link to source documents with page numbers and confidence scores." },
    { icon: Eye, title: "Full Traceability", desc: "Every run is inspectable. View the query, retrieved evidence, confidence levels, and verification status for complete transparency." },
    { icon: Activity, title: "Execution Modes", desc: "Choose the right depth for each question. From instant answers to verified responses for sensitive work." },
    { icon: Shield, title: "Degraded Honesty", desc: "When evidence is weak, the system says so. No confident bluffing — just honest answers about what it can and cannot support." },
    { icon: Building2, title: "Enterprise Scale", desc: "Handle millions of documents across thousands of users. Built for production workloads with flexible deployment options." },
  ];
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          ...
          <div className="rounded-2xl border bg-card p-8 md:p-10">
            ...
            <p className="text-muted-foreground mb-6 leading-relaxed">Most AI answers confidently even when wrong. In many AI systems, the model answers confidently even when the evidence is weak or missing. This creates risk in enterprise contexts where accuracy matters.</p>
          ...
          <div className="rounded-2xl border bg-card p-8 md:p-10">
            ...
            <p className="text-muted-foreground mb-6 leading-relaxed">Vague questions need smarter retrieval. Many questions are vague, multi-part, comparative, or underspecified. Simple retrieval often fails on these harder queries.</p>
            <h4 className="text-sm font-semibold text-foreground mb-3">How Grounded solves this</h4>
            <ul className="space-y-2.5">
              {[
                ...
                return (
                  <li key={i} className="flex items-start gap-2.5 text-sm text-muted-foreground">
                    ...
                    {text as string}
                    {coming && <Badge variant="coming" className="text-[9px] ml-1">Coming soon</Badge>}
                  </li>
                );
              })}
            </ul>
...
function ModesSection() {
  const modes = [
    { name: "Auto", tag: "Recommended", desc: "System chooses the best available execution path. Optimizes for speed and quality based on query complexity.", live: true, icon: Zap },
    { name: "Instant", tag: "Live", desc: "Fast grounded answers for everyday work. Best for straightforward document questions.", live: true, icon: Activity },
    { name: "Thinking", tag: "Coming soon", desc: "Deeper reasoning and retrieval for ambiguity-heavy questions. Multiple retrieval passes and stronger evidence selection.", live: false, icon: Brain },
    { name: "Verified", tag: "Coming soon", desc: "Highest-assurance path for sensitive work. Additional verification and stricter trust controls.", live: false, icon: Shield },
  ];
          ...
          {modes.map((m, i) => (
            ...
            >
              <div className="flex items-center justify-between mb-4">
                ...
                <Badge variant={m.live ? (m.name === "Auto" ? "accent" : "success") : "coming"} className="text-[10px]">
                  {m.tag}
                </Badge>
              </div>
...
function TiersSection() {
  const tiers = [
    { name: "Standard", status: "Live", desc: "Fast grounded baseline optimized for speed, citation quality, and predictable behavior. Powers Auto and Instant modes.", live: true },
    { name: "Enterprise", status: "Coming soon", desc: "Deeper retrieval, better ranking, smarter evidence selection. Designed for harder, more ambiguous questions. Powers Thinking mode.", live: false },
    { name: "Critical", status: "Coming soon", desc: "Highest-assurance path with stronger verification and stricter trust controls. Powers Verified mode.", live: false },
  ];
    ...
    <section className="py-24 md:py-32 bg-secondary/50">
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center max-w-2xl mx-auto mb-16">
          ...
          <p className="text-lg text-muted-foreground">Modes map to execution tiers that determine retrieval depth and verification behavior.</p>
        </div>
          ...
          {tiers.map((t, i) => (
            ...
            >
              <div className="flex items-center gap-3 mb-4">
                ...
                <Badge variant={t.live ? "success" : "coming"} className="text-[10px]">{t.status}</Badge>
              </div>
...
function UseCasesSection() {
  const cases = [
    { num: "01", cat: "Technical Documentation", title: "Customer engineering", desc: "Reliably automate responses to technical inquiries using context from datasheets, call logs, and product data." },
    { num: "02", cat: "Root Cause Analysis", title: "Device log analysis", desc: "Quickly diagnose errors and anomalies in large, complex log files with cited evidence." },
    { num: "03", cat: "Compliance Research", title: "IP & regulatory analysis", desc: "Create detailed reports identifying IP conflicts and compliance gaps based on prior art and regulatory requirements." },
    { num: "04", cat: "Knowledge Management", title: "Enterprise search", desc: "Empower internal teams to find answers fast by searching across scattered knowledge sources." },
    { num: "05", cat: "Report Generation", title: "Qualification reports", desc: "Cross-reference documents across multiple systems to generate audit-ready requirements traceability matrices." },
    { num: "06", cat: "Data Extraction", title: "Data room analysis", desc: "Accurately extract key data from hundreds of messy documents and prepare them for analysis." },
  ];
    ...
    <section id="solutions" className="py-24 md:py-32">
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center max-w-2xl mx-auto mb-16">
          ...
          <h2 className="text-4xl md:text-5xl font-serif text-foreground mb-5">AI smart enough to tackle any expert task</h2>
          <p className="text-lg text-muted-foreground">From technical documentation to compliance research, Grounded handles the work that matters most.</p>
        </div>
    ...
    <section id="pricing" className="py-24 md:py-32 bg-secondary/50">
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center max-w-2xl mx-auto mb-16">
          ...
          <h2 className="text-4xl md:text-5xl font-serif text-foreground mb-5">Start free, scale with your needs.</h2>
        </div>
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {plans.map((p, i) => (
            <motion.div
            ...
            >
              {p.featured && <Badge variant="accent" className="text-[10px] w-fit mb-3">Most popular</Badge>}
              ...
              <ul className="space-y-2.5 mb-8 flex-1">
                {p.modes.map(m => (
                  <li key={m} className="flex items-center gap-2 text-sm text-foreground">
                    <CheckCircle2 className="h-4 w-4 text-accent shrink-0" />{m}
                  </li>
                ))}
              </ul>
              <Button
  ...
  return (
    <section className="py-24 md:py-32 relative overflow-hidden">
      ...
      <div className="max-w-3xl mx-auto text-center px-6 relative">
        ...
        <p className="text-lg text-muted-foreground mb-10 leading-relaxed">Build agents that reason over your data, not around it. Get started with Grounded today.</p>
